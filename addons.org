...
(defun poly/docs-browse-url (url &optional _new-window)
  (set-process-sentinel
   (start-process "docs-brower" "*docs brower*" "osascript" "-e"  (format "tell application \"Firefox\" to open location \"%s\"" url))
   (lambda (proc _)
     (let ((status (process-exit-status proc)))
       (if (= 0 status)
	   (progn
	     (set-process-sentinel
	      (start-process "docs-brower" "*docs brower*" "osascript" "-e"  "tell application \"Firefox\" to activate") nil)
