...
(use-package magit
  ...
  (magit-push-always-verify nil) ;; cuz it says so
...
#+begin_src emacs-lisp
(defvar my/gerrit-target-branches '("main" "master" "develop")
  "Common target branches for `my/gerrit-push'.")

(defun my/gerrit--git-root ()
  "Return the current Git repository root, or signal a user error."
  (or (locate-dominating-file default-directory ".git")
      (user-error "Not inside a Git repository")))

(defun my/gerrit--close-output-window (buffer)
  "Close BUFFER's visible window without killing the buffer."
  (when (buffer-live-p buffer)
    (when-let ((window (get-buffer-window buffer t)))
      (when (> (count-windows) 1)
        (delete-window window)))))

(defun my/gerrit--select-output-window (buffer)
  "Select BUFFER's window, displaying it when needed."
  (when (buffer-live-p buffer)
    (select-window (display-buffer buffer))))

(defun my/gerrit--process-sentinel (process _event)
  "Handle Gerrit push PROCESS completion."
  (when (memq (process-status process) '(exit signal))
    (let ((code (process-exit-status process))
          (branch (process-get process 'my/gerrit-target-branch))
      ...
      (cond
       ((= code 0)
        (message "Gerrit push to %s finished." branch)
        (run-with-timer 0.5 nil #'my/gerrit--close-output-window buffer))
       (t
        (my/gerrit--select-output-window buffer)
        (message "Gerrit push to %s failed. See *Gerrit Push*." branch))))))

(defun my/gerrit-push (target-branch)
  "Push HEAD to Gerrit refs/for/refs/heads/TARGET-BRANCH."
  (interactive
   (list
    (completing-read "Target branch: " my/gerrit-target-branches
		     nil nil nil nil (car my/gerrit-target-branches))))
  (let ((target-branch (string-trim target-branch)))
    (when (string-empty-p target-branch)
      (user-error "Target branch cannot be empty"))
    (save-some-buffers t)
    (let* ((default-directory (my/gerrit--git-root))
	   (refspec (format "HEAD:refs/for/refs/heads/%s" target-branch))
	   (buffer (get-buffer-create "*Gerrit Push*")))
      (with-current-buffer buffer
	(let ((inhibit-read-only t))
	  (read-only-mode -1)
	  (erase-buffer)
	  (insert (format "$ git push origin %s\n\nRunning...\n" refspec))))
      (display-buffer buffer)
      (let ((process
	     (make-process
	      :name "gerrit-push"
	      :buffer buffer
	      :command (list "git" "push" "origin" refspec)
	      :noquery t
	      :sentinel #'my/gerrit--process-sentinel)))
	(process-put process 'my/gerrit-target-branch target-branch)))))

(defmacro my/gerrit-define-push-command (branch)
  "Define an interactive command that pushes HEAD to Gerrit BRANCH."
  (let ((name (intern (format "my/gerrit-push-%s" branch))))
    `(defun ,name ()
       ,(format "Push HEAD to refs/for/refs/heads/%s." branch)
       (interactive)
       (my/gerrit-push ,branch))))

(my/gerrit-define-push-command "main")
(my/gerrit-define-push-command "master")
(my/gerrit-define-push-command "develop")
#+end_src
...
#+begin_src emacs-lisp
(use-package diff-hl
  :vc (:url "https://github.com/dgutov/diff-hl" :branch "master" :rev :newest)
  :ensure t
  :custom-face
...
(defun switch-to-buffer--activate-input-method (&rest _)
  (if (or (string-match "eat-mode" (symbol-name major-mode))
	  (string-match "vterm-mode" (symbol-name major-mode))
...
(advice-add #'switch-to-buffer :after #'switch-to-buffer--activate-input-method)
...
(use-package rime
  ...
  (define-advice rime--text-read-only-p (:around (old-fn) allow-eat-mode)
    (if (eq major-mode 'eat-mode)
        nil
      (funcall old-fn)))

  (define-advice rime--commit (:around (old-fn &rest args) allow-eat-mode)
    (if (eq major-mode 'eat-mode)
	(progn
	  ...
	  (when (bound-and-true-p eat-terminal)
            (eat-term-send-string-as-yank eat-terminal args)))
      (apply old-fn args)))
...
(add-hook 'rime-active-mode-hook
          (lambda () (tl/open-hammerspoon-url "update_input_method")))
...
(use-package eat
  ...
  ;; (eat-shell '("/opt/homebrew/bin/zsh" "-l"))
  ...
  (defun eat-tab-change (original-fun &rest args)
    (interactive)
    ;; (message "eat-tab-change triggered")
       ...
       (with-current-buffer (window-buffer window)
         (when (derived-mode-p 'eat-mode)
           ...
           (when (and (bound-and-true-p meow-global-mode)
             ...
             (meow-insert-mode))
           (when (and (fboundp 'eat-char-mode)
...
(use-package mu4e
  ...
  :load-path "/opt/local/share/emacs/site-lisp/mu4e/";; (expand-file-name "workspace/mu4e/build/mu4e" "~")
...
#+begin_src emacs-lisp
(defun poly/fix-textinput ()
  "Fix textinput."
  (interactive)
  (when-let ((IS-MAC)
	     (dir (poly/project-root)))
    (if dir
	(poly/run-shell-command-in-dir dir (list '("killall" "-HUP" "TextInputMenuAgent")
