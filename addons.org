...
#+begin_src emacs-lisp :tangle no
(use-package tabby
  :vc (:url "https://github.com/shuxiao9058/tabby.git"
	    ;; :rev "d73f488" ;; bad (invalid-read-syntax #<)
	    ;; :rev "36db84b";; bad (invalid-read-syntax #<)
	    ;; :rev "0d4bcbe" ;; fine
	      ;; :rev "f6696fe" ;; bad
	      ;; :rev "e32d259" // error
	      ;; :rev "8e14f6c"
	    ...
	    :rev :newest
	      :branch "main")
    :custom
    (tabby-trigger-mode "auto")
    (tabby-idle-delay 0.5)
    :init
    (defun my/tabby-disable-overlay ()
      "Disable overlays from Tabby after canceling or dismissing"
      (or (member real-last-command '(keyboard-quit tabby-dismiss)) (not (meow-insert-mode-p))))
    (setq tabby-disable-predicates '(my/tabby-disable-overlay window-minibuffer-p)
          tabby-disable-display-predicates '(my/tabby-disable-overlay window-minibuffer-p))
    ...
    (("C-'" . tabby-complete)
     ...
     ("M-f" . tabby-accept-completion-by-word)
     ("M-<return>" . tabby-accept-completion-by-line))
    :commands (tabby-accept-completion)
    :hook (((prog-mode conf-mode text-mode vterm-mode org-msg-edit-mode) . tabby-mode)
	   ;; (meow-insert-exit . (lambda() (tabby--clear-overlay)))
	   )
    ;; :config
    ;; (defun custom/tabby-quit ()
    ;;       "Run `tabby--clear-overlay` or `keyboiard-quit`. If copilot is
    ;; cleared, make sure the overlay doesn't come back too soon."
    ;;       (interactive)
    ;;       (condition-case err
    ;; 	  (when tabby--overlay
    ;; 	    (lexical-let ((pre-tabby-disable-predicates tabby-disable-predicates))
    ;; 			 (setq tabby-disable-predicates (list (lambda () t)))
    ;; 			 (tabby--clear-overlay)
    ;; 			 (run-with-idle-timer
    ;; 			  1.0
    ;; 			  nil
    ;; 			  (lambda ()
    ;; 			    (setq tabby-disable-predicates pre-tabby-disable-predicates)))))
    ;; 	(error handler)))

    ;; (advice-add 'keyboard-quit :before #'custom/tabby-quit)
    )
#+end_src
...
(defun bjodah/minuet-use-localhost-fim ()
  ...
  (plist-put minuet-openai-fim-compatible-options :name "Tabby-localhost-8080")
...
#+begin_src emacs-lisp
(use-package copilot :tangle no
    :vc (:url "https://github.com/copilot-emacs/copilot.el" :branch "main" :rev :newest)
    :hook ((prog-mode conf-mode text-mode vterm-mode org-msg-edit-mode) . copilot-mode)
    :custom
