...
(defvar vitesse-use-italic t
  "Non-nil means use italic for comment and docstring.")

(deftheme vitesse
  "Vitesse theme for Emacs.")

(defgroup vitesse nil
  "Vitesse theme options.

The theme has to be reloaded after changing anything in this group."
  :group 'faces)

(defcustom vitesse-flavor 'light-soft
  "The flavor to use for the vitesse theme.
Must be one of `light`, `light-soft`, `dark`, `dark-soft`, or `black`."
  :type '(choice (const :tag "Vitesse Light" light)
		 (const :tag "Vitesse Light Soft" light-soft)
                 (const :tag "Vitesse Dark" dark)
		 (const :tag "Vitesse Dark Soft" dark-soft)
		 (const :tag "Vitesse Black" black))
  :group 'vitesse)

(defcustom vitesse-enlarge-headings t
  "Use different font sizes for some headings and titles."
  :type 'boolean
  :group 'vitesse)


(defvar vitesse-header-scales
  '(1.6 1.4 1.2 1.2 1.2 1.1 1.0)
  "Scales for headers.")

(defcustom vitesse-height-title-1 1.3
  "Font size 100%."
  :type 'number
  :group 'vitesse)

(defcustom vitesse-height-title-2 1.1
  "Font size 110%."
  :type 'number
  :group 'vitesse)

(defcustom vitesse-height-title-3 1.0
  "Font size 130%."
  :type 'number
  :group 'vitesse)

(defcustom vitesse-height-doc-title 1.44
  "Font size 144%."
  :type 'number
  :group 'vitesse)

(defcustom vitesse-alternate-mode-line-and-minibuffer nil
  "Use less bold and pink in the minibuffer."
  :type 'boolean
  :group 'vitesse)

(defcustom vitesse-light-soft-colors
  '((bg . ("#F1F0E9"))
    (fg . ("#393a34"))
    ;; (fg . ("#998418"))
    (region . ("#222222"))
  ...
  :tag "Vitesse light Colors"
  :options '(bg fg region current comment const number cyan green orange pink purple red yellow white magenta highlight)
...
(defun vitesse--colors()
  "Get colors with vitesse-flavor."
  (pcase vitesse-flavor
    ('light-soft vitesse-light-soft-colors)
    (_ vitesse-light-soft-colors)))

(let ((colors (vitesse--colors))
      (faces '( ;; default
		...
		,@(if vitesse-alternate-mode-line-and-minibuffer
	       ...
	       (vertical-border :foreground ,current)
	       ...
	       (font-lock-function-name-face :foreground ,type :weight bold)
               (font-lock-function-call-face :inherit font-lock-function-name-face)
               (font-lock-punctuation-face :inherit font-lock-bracket-face)
               (font-lock-property-name-face :inherit font-lock-string-face ;; :foreground ,cyan
					     )
               (font-lock-property-use-face  :inherit font-lock-property-name-face)
               (font-lock-regexp-face :inherit font-lock-string-face :underline t)
	       ...
	       (font-lock-string-face :foreground ,namespace)
	       (font-lock-type-face :foreground ,function-name)
	       (font-lock-variable-name-face :foreground ,function-name)
	       ...
	       (font-lock-bracket-face         :foreground ,bracket)
	       (font-lock-escape-face         :foreground ,bracket)
	       (font-lock-property-face         :foreground ,property)
               ...
               (secondary-selection           :background ,brown :extend t)
               ...
               ;; (corfu-annotations            :background ,current)
	        ...
	        ,@(if vitesse-alternate-mode-line-and-minibuffer
               ...
               ;; Emacs Rime
               (rime-preedit-face              :underline ,blue :background ,bg)
               ...
               ;; ;; (org-level-1                    :bold t :height ,(nth 0 vitesse-header-scales))
               ;; ;; (org-level-2                    :bold t :height ,(nth 1 vitesse-header-scales))
               ;; ;; (org-level-3                    :bold t :height ,(nth 2 vitesse-header-scales))
               ;; ;; (org-level-4                    :bold t :height ,(nth 3 vitesse-header-scales))
               ;; ;; (org-level-5                    :bold t :height ,(nth 4 vitesse-header-scales))
               ;; ;; (org-level-6                    :bold t :height ,(nth 5 vitesse-header-scales))
               ;; ;; (org-level-7                    :bold t :height ,(nth 6 vitesse-header-scales))
               ...
               (mu4e-related-face                              :inherit default :slant ,vitesse-use-italic)
	       ...
	       (mu4e-system-face                             :inherit font-lock-comment-face :slant ,vitesse-use-italic)
	       ...
	       (mu4e-compose-separator-face                  :inherit message-separator :slant ,vitesse-use-italic)
	       ;; (mu4e-region-code                             :background "DarkSlateGray")
	       (mu4e-attach-number-face                     :inherit font-lock-variable-name-face :weight bold)
	       (mu4e-compose-header-face                     :inherit message-separator :slant ,vitesse-use-italic)
	       ...
	       (mu4e-view-body-face                          :inherit default)
               ...
               ;; (notmuch-message-summary-face            :foreground ,fg-1)
               (notmuch-search-count                    :foreground ,comment)
               (notmuch-search-date                     :foreground ,purple)
               (notmuch-search-flagged-face             :inherit font-lock-constant-face :weight bold)
               ...
               (notmuch-tree-match-date-face            :foreground ,purple :weight bold)
               ...
               (notmuch-tree-match-tree-face            :foreground ,comment)
               (notmuch-tree-no-match-author-face       :foreground ,blue)
               (notmuch-tree-no-match-date-face         :foreground ,orange)
               ;; (notmuch-tree-no-match-face              :foreground ,bg+1)
               ;; (notmuch-tree-no-match-subject-face      :foreground ,bg+1)
               (notmuch-tree-no-match-tag-face          :foreground ,yellow)
               (notmuch-tree-no-match-tree-face         :foreground ,yellow)
               (notmuch-wash-cited-text                 :foreground ,purple)
               (notmuch-wash-toggle-button :foreground ,fg)
               (notmuch-search-unread-face :foreground "white")
               (notmuch-message-summary-face :background "steel blue" :foreground "snow")
		...
		,@(when vitesse-enlarge-headings
		    (list :height vitesse-height-title-1)))
	       (markdown-header-face-2
		:inherit bold :foreground ,purple
		,@(when vitesse-enlarge-headings
		    (list :height vitesse-height-title-2)))
	       (markdown-header-face-3
		:foreground ,green
		,@(when vitesse-enlarge-headings
		    (list :height vitesse-height-title-3)))
		...
		,@(if vitesse-alternate-mode-line-and-minibuffer
	       ...
	       (org-document-title :inherit font-lock-string-face :weight bold :foreground ,orange
				   ,@(when vitesse-enlarge-headings
				       (list :height vitesse-height-doc-title)))
	       ...
	       (meow-keypad-indicator          :inherit mode-line :foreground ,red)
               ...
               (meow-normal-indicator          :inherit mode-line :foreground ,cursor)
               ...
               (meow-normal-cursor :foreground ,bg  :background ,cursor)
	       ...
	       (apropos-symbol  :inherit font-lock-keyword-face)
               ...
               (cfw:face-day-title           :background ,bg)
               ;; (cfw:face-default-day         :foreground ,base :weight bold)
               (cfw:face-annotation          :foreground ,cyan)
               ;; (cfw:face-disable             :foreground ,fg-1)
               (cfw:face-today-title         :background ,blue :weight bold)
               (cfw:face-today               :background ,orange :weight bold)
               (cfw:face-select              :background ,magenta :weight bold)
               (cfw:face-toolbar             :foreground ,purple :background ,bg)
               ;; (cfw:face-toolbar-button-off  :foreground ,fg-1 :weight bold)
  ...
  (apply #'custom-theme-set-faces
	 'vitesse
	 ;; (make-symbol (concat "vitesse-" (symbol-name vitesse-flavor)))
