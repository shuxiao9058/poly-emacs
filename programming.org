...
(use-package format-all
  ...
  (define-format-all-formatter logos-format
       (:executable (expand-file-name "miniconda3/bin/python" "~"))
       (:install
	(macos "brew install clang-format")
	(windows "scoop install llvm"))
	...
	(format-all--buffer-easy
	 ...
	 "-W"
	 "ignore"
	(expand-file-name "workspace/iOS/logos-format/logos_format/logos_format.py" "~")
        (concat "--assume-filename="
		(expand-file-name ".clang-format" logos-dir))
	 ;; "-i"
	 "--files"
	 (buffer-file-name)
	 )))
...
(use-package flycheck
  ...
  (dolist (lint '(go-errcheck ;; go-unconvert
			      ;; go-staticcheck
		  ;; remove org-lint in case of
		  ;; Error from syntax checker org-lint: Wrong type argument: number-or-marker-p, #("1927" 0 4 (org-lint-marker #<marker at 68699 in addons.org>))
		  org-lint
		  ))
...
(defconst jdt-lombok-jar
  (expand-file-name "jar/lombok.jar" poly-local-dir)
  "Path to the Lombok JAR for Eclipse JDT language server.")
(defconst jdt-extra-jvm-args (list "-noverify"
				   (concat "-javaagent:" jdt-lombok-jar)
				   ;; "-javaagent:[~/.emacs.d/.local/jar/lombok.jar][classes=META-INF/]"
				   (concat  "-Xbootclasspath/a:" jdt-lombok-jar)
...
Examples:
  ...
  (+eglot-register '(kotlin-mode kotlin-ts-mode) `("kotlin-ls" "--stdio"))
  ...
  (with-eval-after-load 'eglot
    ...
    (setq-default eglot-workspace-configuration
		  `(:gopls  (
			    ...
			    :typescript (:format (:indentSize 2 :tabSize 2 :tabSize 2 :convertTabsToSpaces t)
						 :inlayHints
			    ...
			    ;; 	       :autoSearchPaths t
			    ;; 	       :logLevel "Warning"
			    ;; 	       :extraPaths ["/Applications/IDA Professional 9.3.app/Contents/MacOS/python"])))
			    )
		  )
    )
...
use-package will load java-lsp for us simply by calling this function."
    (let ((lombok-jar (expand-file-name "jar/lombok.jar" poly-local-dir)))
...
(use-package clojure-mode
    ...
    :mode (("\\.\\(clj\\|cljd\\|dtm\\|edn\\|lpy\\|bb\\)\\'" . clojure-mode)
	   ("\\.\\(cljs\\|nbb\\)\\'" . clojure-clojurescript-mode)
...
(use-package js
  ...
  (;; (js-mode . lsp)
   (js-mode . (lambda ()
		...
		(local-set-key [remap indent-new-comment-line]
