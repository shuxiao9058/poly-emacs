...
(setq frame-title-format
      '(:eval
	(let ((project-root (poly/project-root))
	      (buffer-name))
	  (if project-root
	      (progn
		(setq buffer-name (replace-regexp-in-string
				   (concat "^" (regexp-quote (expand-file-name (file-name-directory (directory-file-name project-root))))) ""
				   (convert-standard-filename buffer-file-name)))
		(format "%s [%%m] - Emacs" buffer-name))
	    "%b [%m] - Emacs"))))
...
(defun poly/toggle-frame-maximized()
  ...
  (let* ((frame (selected-frame))
	 ...
	 (max-frame-width (- monitor-width 92))
