...
(defvar poly-org-config-file-list
  '(private editor ;; writing
	    programming addons llm)
...
;;     ad-do-it
;;     (move-to-column col)))
(defun poly/scroll-keep-column (orig-fn &rest args)
  "Keep cursor in the same column when scrolling."
  (let ((col (current-column)))
    (apply orig-fn args)
    (move-to-column col)))
(advice-add 'scroll-up   :around #'poly/scroll-keep-column)
(advice-add 'scroll-down :around #'poly/scroll-keep-column)
#+end_src
...
;;         )))
(defun poly/message-add-timestamp (&rest args)
  "Prepend a microsecond timestamp before each message in *Messages*."
  (when-let* (((not (string-equal (car args) "%s%s")))
...
(advice-add 'message :before #'poly/message-add-timestamp)
...
(defun nox/change-colors ()
  ...
  (with-eval-after-load 'meow
    ...
    (set-face-attribute 'meow-beacon-indicator nil  :inherit 'mode-line :foreground (or (catppuccin-get-color 'purple) 'unspecified))
    ...
    (set-face-attribute 'meow-beacon-cursor nil  :inherit 'cursor :background (or (catppuccin-get-color 'purple) 'unspecified)))
