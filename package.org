...
(use-package exec-path-from-shell
  ...
  (if (and poly-enable-native-comp
	...
	(let ((gcc-lib (car (last (file-expand-wildcards "/opt/local/lib/gcc[0-9]*")))))
	  (when gcc-lib
	    (dolist (var '("LIBRARY_PATH" "DYLD_LIBRARY_PATH"))
	      (setenv var (string-join
			   (delete-dups
			    (cons gcc-lib
				  (when (getenv var)
				    (split-string (getenv var) ":" t))))
			   ":")))))
...
(defun poly/message-filter-by-regexp (orig-fn &rest args)
  "Filter messages matching `message-filter-regexp-list' from the echo area.
Matching messages are still silently written to *Messages*."
  (if (null (car args))
      (apply orig-fn args)
    (let ((formatted (apply #'format args)))
      (if (cl-some (lambda (re) (string-match re formatted))
          ...
          (when-let* ((buf (get-buffer "*Messages*")))
            (with-current-buffer buf
              (let ((inhibit-read-only t))
                (goto-char (point-max))
                (insert formatted "\n"))))
        (funcall orig-fn "%s" formatted)))))

(advice-add 'message :around #'poly/message-filter-by-regexp)
